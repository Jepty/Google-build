...
def upload(message, cell_id):
    try:
        ...
        table_results = drive_service.files().list(
            pageSize=1, 
            fields="nextPageToken, files(id, name, mimeType, parents, createdTime)",
            q=f"name contains 'Журнал работ' and '{p_folder_id}' in parents and mimeType='application/vnd.google-apps.spreadsheet'").execute()
...
@bot.message_handler(func=lambda message: lang.butt_free_key_list in message.text and message.chat.id in config.admin_ls, content_types=['text'])
...
def choose_work(call):
    try:
        ...
        results = drive_service.files().list(
            pageSize=2, 
            fields="nextPageToken, files(id, name, mimeType, parents, createdTime)",
            q=f"'{fid}' in parents and mimeType='application/vnd.google-apps.spreadsheet' and name contains 'Журнал работ'").execute()
        if results['files'] == []:
        ...
        elif results['files'][0]['name'] == 'Журнал работ':
            sheet = spread_service.spreadsheets()
...
def add_worker(call):
    ...
    range_mass = f'Список персонала и подрядчиков!B{worker_id}:B{worker_id}'
